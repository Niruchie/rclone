...
func fetchTokenMTProto(ctx context.Context, m *configmap.Mapper) (*fs.ConfigOut, error) {
	...
	if mtproto, err := client.MTProto(); err == nil {
		session := mtproto.
			ExportRawSession().
			Encode()
...
func selectChannelWithBot(ctx context.Context, m *configmap.Mapper) (*fs.ConfigOut, error) {
	// ? Parse the config into the struct
	client := &api.TelegramClient{}
	err := configstruct.Set(*m, &(client.Options))
	if err != nil {
	...
	mtproto, bot, err := client.Connect(ctx)
...
func Configuration(ctx context.Context, name string, m configmap.Mapper, configIn fs.ConfigIn) (*fs.ConfigOut, error) {
	...
	case "":
		return fetchTokenMTProto(ctx, &m)
	case "channel_select":
		return selectChannelWithBot(ctx, &m)
	case "channel_id_set":
